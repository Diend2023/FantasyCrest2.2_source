package
{
   ...
   public class Main extends R4399Main
   {
      ...
      public function getHredImg(param1:String) : BitmapData
      {
         return DataStorage.readImg(param1);
      }
      
      private function loadingload(param1:Event) : void
      {
         var _loc2_:* = null;
         var _loc3_:* = stage.stageWidth / stage.fullScreenWidth;
         if(IS_PHONE)
         {
            stage.scaleMode = StageScaleMode.NO_BORDER;
            stage.align = StageAlign.TOP_LEFT;
            contentWidth = stage.fullScreenWidth * _loc3_;
            contentHeight = stage.fullScreenHeight * _loc3_;
         }
      ...
      private function onLoading(param1:ProgressEvent) : void
      {
         this.loadingmc.gotoAndStop(int(param1.bytesLoaded / param1.bytesTotal * 100));
      }
      ...
      private function onComplete(param1:Event) : void
      {
         ...
         var _loc2_:* = new Timer(500,1);
         _loc2_.addEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayTimeComplete);
         _loc2_.start();
         (this._startSwf.content as MovieClip).stop();
         var _loc3_:* = new ContextMenu();
         var _loc4_:* = new ContextMenuItem("平滑视觉(提高视觉美感)");
         _loc4_.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,this.onSelectHigh);
         var _loc5_:* = new ContextMenuItem("禁用平滑(提高流畅度)");
         _loc5_.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT,this.onSelectLow);
         _loc3_.customItems.push(_loc4_,_loc5_);
         this.contextMenu = _loc3_;
      ...
      private function onSelectHigh(param1:ContextMenuEvent) : void
      {
      ...
      private function onSelectLow(param1:ContextMenuEvent) : void
      {
         stage.quality = StageQuality.LOW;
         if(ComponentResourcesManage.$stage.$box)
         {
      ...
      private function onPlayTimeComplete(param1:TimerEvent) : void
      {
      ...
      private function onPlayVFrame(param1:Event) : void
      {
      ...
      public function com() : void
      {
         ...
         else
         {
            ...
            _loc1_.text = "本游戏有太多的心血，请不要随意转载和破解本游戏，请支持正版，有需要可联系QQ：939029835" + "\n\n" + SwfRestrict.$error;
            _loc1_.x = 0;
            _loc1_.y = 0;
            _loc1_.width = 700;
            _loc1_.height = 500;
            _loc1_.setTextFormat(new TextFormat(null,null,16711680));
      ...
      private function skipTips() : void
      {
         ...
         if(this._startSwf)
         {
            ...
            this._movieTips.setTextFormat(new TextFormat(null,12,16777215,true,null,null,null,null,TextFormatAlign.RIGHT));
      ...
      private function Up(param1:KeyboardEvent) : void
      {
         ...
         {
            P2POnline.send("up","up",param1.keyCode);
         }
         if(param1.keyCode == Keyboard.V)
         {
            GIFManage.start(this,10);
         }
         else if(param1.keyCode == Keyboard.B)
         {
            GIFManage.save();
         }
         else if(param1.keyCode == Keyboard.C)
         {
            GIFManage.stop(this);
         }
      }
      ...
      {
         GameScore.setFight(param1);
      }
      
      private function eixt(param1:Event) : void
      {
      ...
      private function initLogin(param1:Event) : void
      {
         if(this.testMain)
         {
      ...
      private function errorLogin(param1:Event) : void
      {
         if(ComponentResourcesManage.$Ob["gamemain"])
         {
      ...
      private function loginEvent(param1:GameSelectEvent) : void
      {
         var _loc2_:* = null;
         if(param1.clickString == "ann1")
         {
            R4399Main.loginAsk();
         }
         else if(param1.clickString == "hxwz")
         {
      ...
      private function Down(param1:KeyboardEvent) : void
      {
         ...
         var _loc2_:* = null;
         if(param1.keyCode == 4001)
         {
            this.onSelectHigh(null);
            return;
         }
         if(param1.keyCode == 4002)
         {
         ...
         if(SystemP2P.$FightOK)
         {
            P2POnline.send("down","down",param1.keyCode);
            if(Boolean(param1.keyCode == Keyboard.ESCAPE && SystemEvent._playVideoArray) && Boolean(ComponentResourcesManage.$word) && GameData.$fightBoolean)
            {
               this.init2();
            }
            return;
         ...
         {
            this.$keyCode += String(param1.keyCode);
         ...
         else if(param1.keyCode == Keyboard.ENTER && ComponentResourcesManage.$word is SystemWord)
         {
         ...
         else if(param1.keyCode == Keyboard.ESCAPE && GameData.$stageBoolean)
         {
            if(this._movieTips)
            {
            ...
            else if(ComponentResourcesManage.$word is SystemWord && GameData.$fightBoolean)
            {
         ...
         else if(GameData.$1PALLSelect)
         {
            switch(param1.keyCode)
            {
               case Keyboard.Y:
                  ...
                  {
                     _loc4_ = _loc3_.$fightTo - 1;
                     _loc3_.$fightTo = _loc4_;
                  }
                  else
                  {
                     _loc3_.$fightTo -= 2;
                  ...
                  GameScore.init();
                  ComponentResourcesManage.$word.CG();
                  SystemLoadType.loadMAP(_loc3_.$mapArray[int(Math.random() * _loc3_.$mapArray.length)]);
                  SystemEvent.select({"name":_loc3_.$roleSelectArray[0]});
                  break;
               case Keyboard.N:
                  GameData.ns_select = "cg";
                  _loc5_ = GameData.$mode;
                  ComponentResourcesManage.$word.CG();
                  if(_loc5_ == "1PALL" || _loc5_ == "SZ" || _loc5_ == "TwoAll")
                  {
      ...
      public function P2PEvent(param1:NetStatusEvent, param2:NetGroup) : void
      {
         var _loc3_:* = undefined;
         var _loc4_:* = null;
         if(param1.info.code == "NetGroup.Posting.Notify")
         {
            switch(param1.info.message.type)
            {
               case "KOCAONIMA":
                  ComponentResourcesManage.$stage.$box.$api.addShow("草泥马被发现的消息","!!!!!!!!!!!!!!!!!!!!!",true);
                  for(_loc3_ in param1.info.message.array)
                  {
                  }
                  break;
               case "DANMU":
                  if(GameData.$danmu)
                  {
                     _loc4_ = new DanMu(String(param1.info.message.massage),int(param1.info.message.y),int(param1.info.message.speed));
      ...
      public function setNSData(param1:Object) : void
      {
         var _loc2_:* = null;
         NSGameData.onlie = true;
         if(param1)
         {
            GameScore.setFight(int(param1.fight));
            _loc2_ = ComponentResourcesManage.$Ob["fightA"] as TextWord;
            _loc2_.setText = "历史最高战力[" + int(param1.fight) + "]";
         }
         else
         {
      ...
      public function init2(param1:Boolean = true) : void
      {
         if(IS_PREVIEW == true)
         {
      ...
      private function playGoodV() : void
      {
         ...
         ns = new NetStream(connect);
         ns.play("File/indexFile/movie/main_x264.mp3");
         movie = new Video(700,500);
      ...
      private function playEnd(param1:Object) : void
      {
      ...
      private function onMovieFrame(param1:Event) : void
      {
      ...
      private function rankEvent(param1:RankEvent) : void
      {
         ...
         var _loc8_:int = 0;
         var _loc2_:int = this._rankMode == "cg" ? int(R4399Main.cgRankID) : (this._rankMode == "sz" ? int(R4399Main.szRankID) : (this._rankMode == "zh" ? int(R4399Main.zhRankID) : int(R4399Main.srRankID)));
         switch(param1.data)
         {
            case "last":
            case "next":
               R4399Main.rankByPage(_loc2_,this.rank.getPage());
               break;
            case "fj":
               R4399Main.rankByOne(_loc2_);
               break;
            case "fh":
               this.rank.visible = false;
               break;
         ...
         if(param1.data.indexOf("btn") != -1)
         {
            if(this.rank.getMCBoolean())
            {
               this.rank.clearMc();
               _loc3_ = this.rank.getMCData();
               _loc4_ = null;
               if(Boolean(_loc3_) && Boolean(_loc3_.extra))
               {
                  _loc4_ = JSON.parse(P2PData.extract(_loc3_.extra));
               }
               if(Boolean(_loc4_) && Boolean(_loc4_.role))
               {
                  _loc5_ = String(_loc4_.role).split(",");
                  _loc6_ = null;
                  if(_loc4_.rolez)
                  {
                     _loc6_ = String(_loc4_.rolez).split(",");
                  }
                  for(var _loc11_ in _loc5_)
                  {
                     _loc7_ = _loc11_;
                     _loc11_;
                     _loc8_ = _loc6_ ? int(_loc6_[_loc7_]) : 0;
                     if(String(_loc6_ ? _loc6_[_loc7_] : "") == "m")
                     {
                        _loc8_ = 100;
                     }
                     this.rank.addMC(DataStorage.readImg("Hred" + _loc5_[_loc7_]),_loc8_);
                  }
               }
            }
         }
      ...
      private function cheakText(param1:Open4399ToolsEvent) : void
      {
         if(JSON.parse(param1.data).code == 10000)
         {
      ...
      public function onSwfComplete(param1:Event) : void
      {
         var _loc2_:* = (param1.target as LoaderInfo).content;
         this.addChild((param1.target as LoaderInfo).loader);
         var _loc3_:* = _loc2_;
         _loc3_["setContext"](this);
      }
      
      public function gameEvent(param1:GameSelectEvent) : void
      {
         var videoMc:VideoMain = null;
         ...
         {
            case "videos":
               videoMc = new VideoMain(JSON.parse(DataStorage.readText("videos")) as Array);
            ...
            case "playVideo":
               ...
               {
                  (param1.target as FileReference).load();
               });
               file.addEventListener(Event.COMPLETE,function(param1:Event):void
               {
                  var _loc2_:* = param1.target.data as ByteArray;
                  SystemEvent.buildByte(_loc2_,onSwfComplete);
               });
               break;
            case "online":
               SystemEvent.online();
      ...
      private function selectData() : void
      {
         ...
         var _loc1_:XML = GameData.select_xml;
         var _loc2_:int = 0;
         GameData.$allFightArray = [];
         GameData._select_array = [];
         GameData._lock_array = new Object();
         var _loc3_:int = 0;
         while(_loc3_ < _loc1_.select.length())
         {
            _loc4_ = _loc1_.select[_loc3_].role.length();
            if(String(_loc1_.select[_loc3_].@name) !== "怪兽库")
            {
               _loc5_ = 0;
               _loc6_ = R4399Main.getUserData();
               _loc5_ = _loc5_ + _loc6_.cgScore;
               _loc5_ = _loc5_ + _loc6_.szScore;
               _loc5_ = _loc5_ + _loc6_.srScore;
               _loc7_ = 0;
               while(_loc7_ < _loc4_)
               {
                  if(String(_loc1_.select[_loc3_].role[_loc7_].@head) != "NoKown")
                  {
                     GameData.$allFightArray.push(String(_loc1_.select[_loc3_].role[_loc7_].@head));
                  }
                  if(int(_loc1_.select[_loc3_].role[_loc7_].@win) <= _loc5_ || !SwfRestrict.$boolean)
                  {
                     _loc8_ = String(_loc1_.select[_loc3_].role[_loc7_].@head);
                     GameData._select_array.push(String(_loc1_.select[_loc3_].role[_loc7_].@head));
                     if(!GameData.$roleNengLi[_loc8_])
                     {
                        _loc9_ = new Object();
                        _loc9_.hp = _loc1_.select[_loc3_].role[_loc7_].@hp;
                        _loc9_.mp = _loc1_.select[_loc3_].role[_loc7_].@mp;
                        _loc9_.fang = _loc1_.select[_loc3_].role[_loc7_].@fang;
                        _loc9_.li = _loc1_.select[_loc3_].role[_loc7_].@li;
                        _loc9_.kt = _loc1_.select[_loc3_].role[_loc7_].@kt;
                        _loc9_.speed = _loc1_.select[_loc3_].role[_loc7_].@speed;
                        _loc9_.zl = _loc1_.select[_loc3_].role[_loc7_].@zl;
                        _loc9_.jump = _loc1_.select[_loc3_].role[_loc7_].@jump;
                        _loc9_.bq = _loc1_.select[_loc3_].role[_loc7_].@bq;
                        GameData.$roleNengLi[_loc8_] = _loc9_;
                     }
                  }
                  else
                  {
                     GameData._lock_array[String(_loc1_.select[_loc3_].role[_loc7_].@head)] = int(_loc1_.select[_loc3_].role[_loc7_].@win);
                  }
                  _loc7_++;
               }
            }
            else
            {
               ...
               while(_loc7_ < _loc4_)
               {
                  _loc8_ = _loc1_.select[_loc3_].role[_loc7_].@head;
                  if(!GameData.$roleNengLi[_loc8_])
                  {
                     _loc9_ = new Object();
                     _loc9_.hp = _loc1_.select[_loc3_].role[_loc7_].@hp;
      ...
      public function init() : void
      {
         ...
         var _loc1_:* = new TextWord("未登录",new Point(110,48),new Rectangle(0,0,295,24));
         ComponentResourcesManage.$Ob["rank"] = _loc1_;
         ComponentResourcesManage.$stage.addChild(_loc1_);
         _loc1_.$textForm.align = TextFormatAlign.RIGHT;
         _loc1_.setColor = 16776960;
         var _loc2_:* = new TextWord("周排行 - NO.10000    月排行 - NO.10000",new Point(35,395),new Rectangle(0,0,293,24));
         ComponentResourcesManage.$Ob["rankA"] = _loc2_;
         ComponentResourcesManage.$stage.addChild(_loc2_);
         _loc2_.$textForm.align = TextFormatAlign.LEFT;
         _loc2_.setColor = 16776960;
         var _loc3_:* = new TextWord("历史最高战力[0]",new Point(263,395),new Rectangle(0,0,136,24));
         ComponentResourcesManage.$Ob["fightA"] = _loc3_;
         ComponentResourcesManage.$stage.addChild(_loc3_);
         _loc3_.$textForm.align = TextFormatAlign.RIGHT;
         _loc3_.setColor = 16776960;
      ...
      {
         var _loc1_:* = new GameHelp();
         this.addChild(_loc1_);
      }
      
      public function updateRank(param1:Object) : void
      {
         var _loc2_:* = null;
         var _loc3_:* = null;
         var _loc4_:* = null;
         this._rank_ob = param1;
         if(ComponentResourcesManage.$Ob["rank"])
         {
            _loc2_ = ComponentResourcesManage.$Ob["rank"];
            _loc3_ = ComponentResourcesManage.$Ob["rankA"];
            _loc4_ = ComponentResourcesManage.$Ob["fightA"];
            if(param1)
            {
               _loc3_.setText = "月排行 - NO." + (int(param1.yue) == 0 ? "无" : param1.yue) + "    周排行 - NO." + (int(param1.zhou) == 0 ? "无" : param1.zhou);
               _loc2_.setText = "纹章使[" + param1.name + "] 战力[" + param1.fight + "] 你的日排名[" + (param1.rank == 0 ? "无" : param1.rank) + "]";
               _loc4_.setText = "历史最高战力[" + GameScore.getFight() + "]";
               _loc2_.visible = true;
            }
            else
            {
            ...
            ComponentResourcesManage.$Ob["LOG"].visible = !_loc2_.visible;
         }
      }
      ...
      public function TimeLast() : void
      {
         ...
         else if(GameData.$game_time == 120)
         {
            GameData.$game_time = 0;
         }
      ...
      public function SoudStop() : void
      {
         ...
         {
            _loc1_ = SystemMucisPlay;
            _loc2_ = _loc1_.$mode - 1;
            _loc1_.$mode = _loc2_;
            ComponentResourcesManage.$Ob["Sound"].setText = this.SoundData(_loc1_.$mode);
         }
      }
      ...
      private function SoundData(param1:int) : String
      {
         switch(param1)
         {
            case 0:
